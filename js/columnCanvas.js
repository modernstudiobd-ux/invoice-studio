...
export function renderColumnManagerList() {
  ...
  state.columns.forEach((c, idx) => {
    ...
    row.innerHTML = `<div class="columnhead"><button class="btn icon reorder" data-dir="up" aria-label="Move column ${idx + 1} up" ${idx === 0 ? "disabled" : ""}>↑</button><button class="btn icon reorder" data-dir="down" aria-label="Move column ${idx + 1} down" ${idx === state.columns.length - 1 ? "disabled" : ""}>↓</button><span class="drag" aria-hidden="true">☰</span><strong>Column ${idx + 1}</strong><label class="tiny" for="${uidStr}-vis"><input id="${uidStr}-vis" class="vis" type="checkbox" ${c.visible ? "checked" : ""}> Show</label><button class="btn icon danger" aria-label="Remove column ${idx + 1}" title="Remove column">×</button></div><div class="colgrid"><div class="field"><label for="${uidStr}-label">Heading</label><input id="${uidStr}-label" class="labelinput" value="${esc(c.label)}"></div><div class="field"><label for="${uidStr}-type">Type</label><select id="${uidStr}-type"><option value="text">Text</option><option value="number">Number</option><option value="currency">Currency</option><option value="percentage">Percentage</option><option value="date">Date</option></select></div><div class="field"><label for="${uidStr}-width">Width %</label><input id="${uidStr}-width" class="width" type="number" min="5" max="80" value="${c.width}"></div><div class="field"><label for="${uidStr}-align">Alignment</label><select id="${uidStr}-align"><option value="left">Left</option><option value="right">Right</option><option value="center">Center</option></select></div><div class="field"><label for="${uidStr}-role">Calculation role</label><select id="${uidStr}-role"><option value="none">None</option><option value="quantity">Quantity</option><option value="rate">Rate</option><option value="amount">Amount</option></select></div></div>`;
